...
if len(inh) > 0:
    ...
    if pick == 0:
        ...
        warteansage  = 'keine Änderung in der letzten Minute'
    else:
        ...
        bolusansage2 = 'units'
        warteansage  = 'same procedure as last minute'

    inh = glob.glob(test_dir+'files/*.log')
    fn = inh[0]
    ClearScreenCommand = 'clear'
    maxItem = '14'    # shows last hour; was: maxItem = input('How many items to list (14)? ')
...
while wdhl[0]=='y':                                                                 # use CANCEL to stop/exit
    ...
    print ("\n time      ---- predicted BG's ---  --carbs req--"
         ...
         + "    COB  IOB   Req  maxSMB SMB  Req  Safe temp")
    ...
    for thisTime in sorted_entries[len(sorted_entries)-top10:]:                     # last 10 entries
        ...
        print (f'{thisTime[11:16]}Z {values["bg"]:>3}{values["minPredBG"]:>5} '
             ...
             + f'{values["CarbReqGram"]:>5}  {values["CarbReqTime"]:>3}   '
             + f'{round(values["COB"],1):>4}{round(values["iob"],1):>5} '
